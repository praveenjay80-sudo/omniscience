...
export async function POST(req: NextRequest) {
  ...
  const prompt = `You are a brilliant intellectual historian. Trace the complete intellectual genealogy of ${context} — the unbroken chain of thinkers and ideas that built this field, generation by generation.
...
**Key works:** 3–4 essential works with search links: [Title](https://www.amazon.com/s?k=Title+Author) for books, [Title](https://scholar.google.com/scholar?q=Title+Author) for papers. Encode spaces as +.
...
- Show the **surprising parentages** — ideas we think of as modern that are secretly ancient
- Include the **heretics** whose ideas were rejected but later vindicated
- Trace from the earliest roots all the way to the living generation — do not stop early
